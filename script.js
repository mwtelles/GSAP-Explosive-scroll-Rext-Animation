document.addEventListener("DOMContentLoaded", () => {
    ...
    const wordsToHightlight = words.filter((word) => highlightWords.some((highlightWord) => word.textContent.includes(highlightWord)));
    ...
    wordsToHightlight.forEach((word) => {
        ...
        scrollTrigger: {
            ...
            onUpdate: (self) => {
                ...
                if (self.progress >= 0.6 && !physicsEnabled && isScrollingDown) {
                    ...
                    gsap.to(
                        words.filter((word) => !highlightWords
                            .some((hw) => word.textContent.includes(hw))
                        ),
                        {
                            opacity: 0,
                            duration: 0.5,
    ...
    shuffledHighlights = [...wordsToHightlight];
    ...
    tl.add(phaseOne, 0).add(phaseTwo, 1).to({}, {
        duration: 2
    });
